...
public interface IFeedAdministrationService
{
    ...
    Task<(string FilePath, string DownloadFileName)?> GetNuGetPackageFileAsync(string packageId, string version, CancellationToken ct = default);
    Task<List<string>> GetHostedNuGetVersionsAsync(string packageId, CancellationToken ct = default);
}
